...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    // Order within the block: [tool skill cards] [user skills]
    // [research directive]. The directive comes LAST by design — small models
    // show strong recency bias and the per-task instruction is what we want
    // freshest in their attention. Delivered at the conversation tail (see
    // _shared/inject.ts), which is later still than the end of the system
    // prompt.
    const block = skillBlock + userSkillsBlock + directive;
