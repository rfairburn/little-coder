...
- **Sub-coders (`dispatch`)** — little-coder can spawn isolated child sessions to research a question (read the repo + browse online, read-only) and report back concisely, without cluttering the main conversation. A live panel above the input tracks them. Use `/dispatch-settings` to choose their model, reasoning level, and serial or parallel execution; Plan Mode and Deep Research inherit the same sub-coder settings. Until a mode is saved, sub-coders remain serial by default and `LITTLE_CODER_SUBCODER_CONCURRENCY=2` or more remains the parallelism override.
...
│       ├── subagent/              # `dispatch` tool + shared sub-coder settings, read/browse-only spawn engine, and live tracker
